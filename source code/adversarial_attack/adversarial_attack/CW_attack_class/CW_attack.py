...
class AttackCarliniWagnerL2:
    ...
    def __init__(self, normalizer, targeted=True, search_steps=None, max_steps=None, cuda=True, debug=False):
        self.debug = debug
        self.targeted = targeted
        self.num_classes = 10
        self.confidence = 0.  # FIXME need to find a good value for this, 0 value used in paper not doing much...
        self.initial_const = 1.  # bumped up from default of .01 in reference code
        ...
        #self.normalize_dict = {"mean" : [0.485, 0.456, 0.406], "std" : [0.229, 0.224, 0.225]}
        self.normalizer = normalizer
        self.one_img_dim = float(3*32*32)
    ...
    def _optimize(self, optimizer, model, input_var, modifier_var, target_var, scale_const_var, input_orig=None):
        ...
        #os._exit(0)
        output = model(self.normalizer(input_adv))
        ...
        #return normalized input_adv_np
        input_adv_np = copy.deepcopy(input_adv.data.permute(0, 2, 3, 1).cpu().numpy())  # back to BHWC for numpy consumption
    ...
    def run(self, model, input, target, batch_idx=0):
        ...
        if self.clamp_fn == 'tanh':
            ...
            input_var = autograd.Variable(torch_arctanh(input,self.clip_min,self.clip_max,1e-8), requires_grad=False)
        ...
        target_onehot = torch.zeros(target.size(0),self.num_classes)
        if self.cuda:
        ...
        optimizer = optim.Adam([modifier_var], lr=0.001)
        #optimizer = optim.SGD([modifier_var], lr=0.1, momentum=0.9)
        ...
        for search_step in range(self.binary_search_steps):
            ...
            for step in range(self.max_steps):
                ...
                loss, dist, output, adv_img = self._optimize(
                    ...
                    input_orig)
                '''
                temp_path = "/media/trs1/TheVirtuoso/BNN/BNN_attack/toy/"
                if os.path.exists(temp_path) == False:
                    os.mkdir(temp_path)
                with open(temp_path+str(step)+'.pkl','wb') as f:
                    pickle.dump(adv_img, f, pickle.HIGHEST_PROTOCOL)
                '''
                if step % 100 == 0 or step == self.max_steps - 1:
                    print('Step: {0:>4}, loss: {1:6.4f}, dist_loss: {5:6.4f} ,average dist: {2:8.5f}, pixel_wise_L2: {3:8.5f}, modifier mean: {4:.5e}'.format(
                        step, loss, dist.mean(), dist.mean()/self.one_img_dim ,torch.abs(modifier_var.data).mean(),dist.sum()))
