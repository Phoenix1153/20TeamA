...
#input files name
train_file = os.path.join(target_dir, "predict-v1.json")
dev_file = os.path.join(target_dir, "predict-v1.json")
predict_file = os.path.join(target_dir, "predict-v1.json")
...
checkpoint = 20 #checkpoint to save and evaluate the model
period = 20 #period to save batch loss
val_num_batches = 40 #Number of batches to evaluate the model
test_num_batches = 40 #Number of batches to evaluate the model
predict_num_batches = 40
...
glove_char_file = os.path.join(target_dir, "glove.840B.300d-char.txt")
pretrained_char = False #Whether to use pretrained char embedding

fasttext_file = os.path.join(target_dir, "wiki-news-300d-1M.vec")
