...
def train(config, model, device, optimizer, scheduler, ema, dataset, start, length):
    model.train()
    losses = []
...
def valid(config, model, device, dataset, eval_file):
    model.eval()
    answer_dict = {}
    losses = []
...
def test(config, model, device, dataset, eval_file):
    model.eval()
    answer_dict = {}
    losses = []
...
def predict(config, model, device, dataset, eval_file):
    model.eval()
    answer_dict = {}
    losses = []
...
def QATrain(config, model, device):
    ...
    for iter in range(0, N, L):
        train(config, model, device, optimizer, scheduler, ema, train_dataset, iter, L)
        valid(config, model, device, train_dataset, train_eval_file)
        metrics = test(config, model, device, dev_dataset, dev_eval_file)
        ...
        torch.save({'static_dict':model.state_dict()}, fn)

def QAPredict(config, model, device):
    with open(config.predict_eval_file, "r") as fh:
        predict_eval_file = json.load(fh)
    predict_dataset = SQuADDataset(config.predict_record_file, 1, config.predict_num_batches)
    #fn = os.path.join(config.save_dir, "model.pt")
    #model = torch.load(fn)
    predict(config, model, device, predict_dataset, predict_eval_file)
