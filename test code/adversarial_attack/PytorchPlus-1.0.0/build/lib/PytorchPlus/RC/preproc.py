...
class Preproc():
    ...
    def convert_to_features(self,config, data, word2idx_dict, char2idx_dict):
        example = {}
        context, question = data
        context = context.replace("''", '" ').replace("``", '" ')
        question = question.replace("''", '" ').replace("``", '" ')
        example['context_tokens'] = self.word_tokenize(context)
        example['ques_tokens'] = self.word_tokenize(question)
