...
class Embedding(nn.Module):
    ...
    def forward(self, ch_emb, wd_emb):
        dropout_char = self.dropout_char
...
class EncoderBlock(nn.Module):
    def __init__(self, config, conv_num: int, ch_num: int, k: int, length: int):
        super().__init__()
        d_model = config.d_model
        self.dropout = config.dropout
        ...
        self.pos = PosEncoder(config, length)
...
class QANet(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.c_emb_enc = EncoderBlock(config, conv_num=4, ch_num=d_model, k=7, length=len_c)
        self.q_emb_enc = EncoderBlock(config, conv_num=4, ch_num=d_model, k=7, length=len_q)
        self.cq_att = CQAttention(config)
        self.cq_resizer = DepthwiseSeparableConv(d_model * 4, d_model, 5)
        enc_blk = EncoderBlock(config, conv_num=2, ch_num=d_model, k=5, length=len_c)
        self.model_enc_blks = nn.ModuleList([enc_blk] * 7)
        self.out = Pointer(config)
